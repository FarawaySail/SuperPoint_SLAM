...
def writeSPtofile(pts, desc,sizeratio, filename_pre, maxpoints = 1000):
  maxlen = min( pts.shape[1] , maxpoints)
  pts = pts[:,0:maxlen]
  outpts_x = pts[0]*sizeratio[0]
  outpts_y = pts[1]*sizeratio[1]
  desc = desc[:,0:maxlen]
  with open(filename_pre+'_kpts.txt','w') as outfile:
    for index in range(0,len(outpts_x) ):
      print('{x} {y}'.format( x=int(outpts_x[index]),y=int(outpts_y[index]) ),file=outfile)
  with open(filename_pre+'_feats.txt','w') as outfile:
    for index in range(0,desc.shape[1] ):
      for element in desc[:,index]:
        print(element,file=outfile,end=' ')
      print('',file=outfile)


def getImage2SP(img_name, filename_pre, fe):
  grayim = cv2.imread(img_name,0)
  ori_size = grayim.shape
  interp = cv2.INTER_AREA
  grayim = cv2.resize(grayim, ( 480 , 640 ), interpolation=interp)
  sizeratio = (ori_size[0]/480 , ori_size[1]/640)
  grayim = (grayim.astype('float32') / 255.)
  pts, desc, _ = fe.run(grayim)
  writeSPtofile(pts,desc,sizeratio,filename_pre)
  tmp = 1

def test2SP(img_name, fe):
  grayim = cv2.imread(img_name,0)
  ori_size = grayim.shape
  interp = cv2.INTER_AREA
  grayim = cv2.resize(grayim, ( 480 , 640 ), interpolation=interp)
  ...
  for index in range(0,len(outpts_x) ):
    cv2.circle(grayim, (int(outpts_x[index]),int(outpts_y[index])), 1, (0, 255, 0), -1, lineType=16 )
  cv2.imshow("Test",grayim)
  cv2.waitKey()
...
if __name__ == '__main__':
  ...
  listenlingfolder = '/dev/shm/tmpSP/'
